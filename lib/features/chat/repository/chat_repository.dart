...
class ChatRepository implements IChatRepository {
  ...
  Future<Iterable<ChatMessageDto>> sendMessage(String message) async {
    ...
    await _client.sendMessage(SjMessageSendsDto(text: message));
  ...
  Future<Iterable<ChatMessageDto>> _fetchAllMessages() async {
    ...
    while (!isLimitBroken) {
      final batch =
          await _client.getMessages(lastMessageId: lastMessageId, limit: 10000);
    ...
    final users =
        await _client.getUsers(messagesWithUsers.values.toSet().toList());
    final localUser = await _client.getUser();
